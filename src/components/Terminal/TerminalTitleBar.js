...
export default function TerminalTitleBar({
  ...
  function terminalTab(location) {
    return (
      <div
        className={`bg-black w-[240px] p-[5px] rounded-t flex text-xs items-center  mt-[5px]`}
        onClick={() => handleChangeLocation(location.location)}
      >
        <div
        ...
        <div className="text-white whitespace-nowrap overflow-hidden font-['Segoe_UI']">
          {`derek@DMR-DESKTOP: ${location.location}`}
        </div>
  ...
  return (
    <div className="h-[36px] pl-[7px] pr-[18px] flex justify-between bg-terminal-titleBar">
      ...
      <div className="flex items-center">
        {terminalLocations.map((location) => terminalTab(location))}
        {/* {terminalTab} */}
